...
# --- ЯДРО СИСТЕМЫ ---
@st.cache_data
def initial_process_dataset(df):
...
def recalculate_dynamic_metrics(df, classes):
...
def assign_status(row):
    ...
    # ИСПРАВЛЕНО: Хаос - это нормализованная энтропия > 60% (а не жестко 1.0)
    ...
    if proj_data['data'] is None:
        ...
        if uploaded_file is not None:
            ...
            with st.spinner("Сборка тензоров..."):
                processed_df, extracted_classes = initial_process_dataset(raw_df)
                final_df = recalculate_dynamic_metrics(processed_df, extracted_classes)
                st.session_state.projects[active_name]['data'] = final_df
                st.session_state.projects[active_name]['classes'] = extracted_classes
                _, readiness_lvl = calculate_dataset_metrics(final_df, extracted_classes)
        ...
        # --- ВКЛАДКА 2: ОЧЕРЕДЬ И КАРТОЧКА ОБЪЕКТА (ПОЛНАЯ ИНФОРМАЦИЯ) ---
        with tab2:
            if role == "Разметчик":
                st.markdown("<span style='color: #f7768e;'>Доступ закрыт.</span>", unsafe_allow_html=True)
            else:
        ...
        with tab3:
            if role in ["Аналитик данных", "ML-инженер"]:
                ...
                if not edit_subset.empty:
                    ...
                    if changed:
                        df = recalculate_dynamic_metrics(df, classes)
        ...
        with tab4:
            ...
            if role == "Администратор проекта":
                with st.expander("Добавить новые данные (Загрузка нового CSV)"):
                    ...
                    if new_file is not None:
                        ...
                        processed_df, ext_classes = initial_process_dataset(raw_df)
                        final_df = recalculate_dynamic_metrics(processed_df, ext_classes)
            ...
            display_df = df.copy()
            display_df['Статус'] = display_df.apply(assign_status, axis=1)
            # ДОБАВЛЕНЫ КОЛОНКИ УВЕРЕННОСТИ И ЭНТРОПИИ, УБРАН ЛИМИТ
